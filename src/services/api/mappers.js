...
export function mapServiceOrder(order) {
  ...
  return {
    ...
    estimatedDurationMinutes: order.estimatedDurationMinutes != null
      ? Number(order.estimatedDurationMinutes)
      : null,
    ...
    extras: (order.extras || []).map((extra) => ({
      ...
      defaultPrice: Number(extra.serviceExtra?.defaultPrice || extra.price || extra.priceAtTime || 0),
      estimatedTime: extra.serviceExtra?.estimatedTime || extra.estimatedTime,
      source: extra.source || 'provider_field',
